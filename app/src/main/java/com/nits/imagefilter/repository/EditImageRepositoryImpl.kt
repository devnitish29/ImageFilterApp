package com.nits.imagefilter.repository
...
class EditImageRepositoryImpl(private val context: Context) : EditImageRepository {
    ...
    override suspend fun saveFilteredImage(filteredBitmap: Bitmap): Uri? {
        return try {
            val mediaStorageDirectoy = File(
                context.getExternalFilesDir(Environment.DIRECTORY_PICTURES),
                "Saved Images"
            )
            if (!mediaStorageDirectoy.exists()) {
                mediaStorageDirectoy.mkdirs()
            }

            val fileName = "IMG_${System.currentTimeMillis()}.jpg"
            val file = File(mediaStorageDirectoy, fileName)
